...
# 复制 Next.js standalone 构建输出 (copy contents directly, including hidden .next directory)
if [ -d ".next/standalone" ]; then
    echo "  - 复制 .next/standalone 内容到 next-service-dist"
    mkdir -p "$BUILD_DIR/next-service-dist"
    cp -r .next/standalone/* "$BUILD_DIR/next-service-dist/"
    # Copy hidden .next directory explicitly
    if [ -d ".next/standalone/.next" ]; then
        echo "  - 复制 .next/standalone/.next"
        cp -r .next/standalone/.next "$BUILD_DIR/next-service-dist/"
    fi
fi
...
if [ -f ".env" ]; then
    echo "  - 复制 .env"
    cp .env "$BUILD_DIR/next-service-dist/"
fi

# 注意: 不需要再复制 .next/static，因为 standalone 已经包含了
